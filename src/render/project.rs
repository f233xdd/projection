...
use super::{approximate, graphcalc, p, pn};
use graphcalc::{
    geo2d, geo3d,
    interface::{CalcAngle, CalcDistance},
};
            ...
            if !spec {
                if z_s > 0.0 {
                    y = -d
                        * cal_cos_by_point(
                            &p!(
                                x_s + z_s.powi(2) * x_s / (x_s.powi(2) + y_s.powi(2)),
                                y_s + z_s.powi(2) * y_s / (x_s.powi(2) + y_s.powi(2)),
                                0.0
                            ),
                            &p!(x_s1, y_s1, z_s1),
                            &p!(x_s, y_s, z_s),
