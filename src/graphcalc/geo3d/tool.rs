// 3D part
use super::{
    super::{algebra::calc::approximate, algebra::vector::SpaceVec, geo_err},
    component::{Line, Plane, Point},
};
...
pub fn calc_line_fn(
    ...
    if (x_p2 != x_p1) || (y_p2 != y_p1) || (z_p2 != z_p1) {
        if !approximate(x_p1, x_p2) {
            Ok((
                (y_p1 - y_p2, x_p2 - x_p1, 0.0, x_p2 * y_p1 - x_p1 * y_p2),
                (z_p1 - z_p2, 0.0, x_p2 - x_p1, x_p2 * z_p1 - x_p1 * z_p2),
            ))
        } else {
            Ok((
                (0.0, z_p1 - z_p2, y_p2 - y_p1, y_p2 * z_p1 - y_p1 * z_p2),
                (1.0, 0.0, 0.0, x_p1),
            ))
        }
    } else {
...
/// k1 * x + k2 * y + k3 * z = b
pub fn calc_plane_fn(
    p1: &Point,
    p2: &Point,
    p3: &Point,
) -> Result<(f64, f64, f64, f64), geo_err::InvalidFnArgError> {
    let (x_p1, y_p1, z_p1) = p1.pos();
    let (x_p2, y_p2, z_p2) = p2.pos();
...
pub fn line_is_in_plane(ln: &Line, pn: &Plane) -> bool {
    ...
    approximate(pn.get_normal_vec() * ln.get_direction_vec(), 0.0)
        && approximate(
            b * (k11 * k22 - k12 * k21),
            b1 * (k1 * k22 - k2 * k21) - b2 * (k1 * k12 - k2 * k11),
        )
}
...
pub fn line_is_parallel(ln1: &Line, ln2: &Line) -> bool {
    ...
    approximate(
        (k113 * k122 - k112 * k123) * (k211 * k223 - k213 * k221),
        (k213 * k222 - k212 * k223) * (k111 * k123 - k113 * k121),
    ) && approximate(
        (k111 * k123 - k113 * k121) * (k212 * k221 - k211 * k222),
        (k211 * k223 - k213 * k221) * (k112 * k121 - k111 * k122),
...
pub fn point_is_superposition(p1: &Point, p2: &Point) -> bool {
    let (x_p1, y_p1, z_p1) = p1.pos();
    let (x_p2, y_p2, z_p2) = p2.pos();
    approximate(x_p1, x_p2) && approximate(y_p1, y_p2) && approximate(z_p1, z_p2)
}
...
pub fn line_is_superposition(ln1: &Line, ln2: &Line) -> bool {
    ...
    let ((a21, b21, c21, d21), (a22, b22, c22, d22)) = ln2.fn_args();
    approximate(
        (c11 * b21 - b11 * c21) * (a21 * c22 - c21 * a22),
        (c21 * b22 - b21 * c22) * (a11 * c21 - c11 * a21),
    ) && approximate(
        (a11 * c21 - c11 * a21) * (b21 * a22 - a21 * b22),
        (a21 * c22 - c21 * a22) * (b11 * a21 - a11 * b21),
    ) && approximate(
        a21 * ((d11 - c11) * b12 - (d12 - c12) * b11)
            - b21 * ((d11 - c11) * a12 - (d12 - c12) * a11)
            + (c21 - d21) * (a11 * b12 - a12 * b11),
        0.0,
    ) && approximate(
        a22 * ((d11 - c11) * b12 - (d12 - c12) * b11)
            - b22 * ((d11 - c11) * a12 - (d12 - c12) * a11)
            + (c22 - d22) * (a11 * b12 - a12 * b11),
        0.0,
    )
}
...
pub fn calc_point_line_d(p: &Point, ln: &Line) -> f64 {
    ...
    (((b * x_p - a * y_p - d3).powi(2)
        + (a * z_p - c * x_p - d2).powi(2)
        + (c * y_p - b * z_p - d1).powi(2))
...
pub fn calc_line_d(ln1: &Line, ln2: &Line) -> f64 {
    ...
    if line_is_parallel(ln1, ln2) {
        ...
        (lbd * d21 - d11).powi(2)
            + (lbd * d22 - d12).powi(2)
            + (lbd * d23 - d13).powi(2) / (a1.powi(2) + b1.powi(2) + c1.powi(2))
    } else {
        ...
        (k111 * (k122 * (k213 * b22 - k223 * b21) + k123 * (k222 * b21 - k212 * b22))
            ...
            + k213 * (k221 * (k112 * b12 - k122 * b11) + k222 * (k121 * b11 - k111 * b12)))
            .abs()
            / ((b1 * c2 - b2 * c1).powi(2)
                + (a2 * c1 - a1 * c2).powi(2)
                + (a1 * b2 - a2 * b1).powi(2))
...
pub fn calc_line_plane_d(ln: &Line, pn: &Plane) -> Result<f64, geo_err::NotParallelError> {
    ...
    if line_plane_is_parallel(ln, pn) {
        ...
        if !approximate(a, 0.0) {
            Ok(
                (k2 * (k23 * b1 - k13 * b2) / a + k3 * (k12 * b2 - k22 * b1) / a - b).abs()
                    / (k1.powi(2) + k2.powi(2) + k3.powi(2)).sqrt(),
            )
        } else {
            a = k11 * k23 - k13 * k21;
            if !approximate(a, 0.0) {
                Ok(
                    (k1 * (k23 * b1 - k13 * b2) / a + k3 * (k11 * b2 - k21 * b1) / a - b).abs()
                        / (k1.powi(2) + k2.powi(2) + k3.powi(2)).sqrt(),
                )
            } else {
                a = k11 * k22 - k12 * k21;
                Ok(
                    (k1 * (k22 * b1 - k12 * b2) / a + k2 * (k11 * b2 - k21 * b1) / a - b).abs()
                        / (k1.powi(2) + k2.powi(2) + k3.powi(2)).sqrt(),
                )
            }
        }
...
pub fn calc_line_intersection(ln1: &Line, ln2: &Line) -> Result<Point, geo_err::ParallelError> {
    ...
    if line_is_coplanar(ln1, ln2) && !line_is_parallel(ln1, ln2) {
        let a = (k111 * k123 - k113 * k121) * (k212 * k223 - k213 * k222)
            - (k211 * k223 - k213 * k221) * (k112 * k123 - k113 * k122);
        let (x, y, z) = (
            ((k123 * b11 - k113 * b12) * (k212 * k223 - k213 * k222)
                - (k223 * b21 - k213 * b22) * (k112 * k123 - k113 * k122))
                / a,
            ((k123 * b11 - k113 * b12) * (k211 * k223 - k213 * k221)
                - (k223 * b21 - k213 * b22) * (k111 * k123 - k113 * k121))
                / -a,
            ((k121 * b11 - k111 * b12) * (k213 * k222 - k212 * k223)
                + (k122 * b11 - k112 * b12) * (k211 * k223 - k213 * k221)
                + (k223 * b21 - k213 * b22) * (k112 * k121 - k111 * k122))
                / a,
        );
        Ok(Point::new(x, y, z))
    } else {
...
pub fn calc_line_plane_intersection(
    ln: &Line,
    pn: &Plane,
) -> Result<Point, geo_err::ParallelError> {
    let ((k11, k12, k13, b1), (k21, k22, k23, b2)) = ln.fn_args();
    let (k1, k2, k3, b) = pn.fn_args();
    let k = ln.get_direction_vec() * pn.get_normal_vec();
    if !approximate(k, 0.0) {
        Ok(Point::new(
            (k2 * (k13 * b2 - k23 * b1) + k3 * (k22 * b1 - k12 * b2) + (k12 * k23 - k13 * k22) * b)
                / -k,
            (k1 * (k13 * b2 - k23 * b1) + k3 * (k21 * b1 - k11 * b2) + (k11 * k23 - k13 * k21) * b)
                / k,
            (k2 * (k11 * b2 - k21 * b1) + k1 * (k22 * b1 - k12 * b2) + (k12 * k21 - k11 * k22) * b)
                / k,
        ))
    } else {
        Err(geo_err::ParallelError())
...
pub fn calc_plane_intersection(
    pn1: &Plane,
    pn2: &Plane,
) -> Result<Line, geo_err::InvalidFnArgError> {
    let (k11, k12, k13, b1) = pn1.fn_args();
    let (k21, k22, k23, b2) = pn2.fn_args();
